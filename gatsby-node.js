...
exports.createPages = async ({ actions, graphql, reporter }) => {
  ...
  let pages = _.difference(nodes, _.flatten(grouped.map(g => g.nodes))).map(
    n => {
  ...
  grouped.forEach(group => {
    ...
    group.nodes[0].html = group.nodes
      .map(page => {
        ...
        const fragment = page.frontmatter.path.replace(
          /\//g, // remove slashes
          ''
        )

        // TODO error if <h1> is not the first child, verify fragment is unique
        // replace fragment generated by autolink plugin with the path from the markdown file
        $('h1')
          .first()
          .attr('id', fragment)
          .find('a')
        ...
        return `<section>${$.html()}</section>` // content of each markdown file gets its own section
      })
      .join('')
